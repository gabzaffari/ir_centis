...
read_ir_clean <- function(path, sheet){
  ...
  df <-  suppressWarnings(
    ...
    rename(centil_1 = "x1",
           ...
           divids_onus = "x24") %>%
    slice(2:n()) %>%
    mutate(centil_2 = 0.1*centil_2+99,
...
read_ir_sheets <- function(path){
  ...
  df_ano <- purrr::map2(path, sheets, read_ir_clean) %>%
    set_names(sheets) %>%
    purrr::map2(sheets, ~ mutate(.x, tabela = case_when(
      str_length(.y) == 3 ~ "RTB",
      str_length(.y) == 4 ~ "RB1",
      str_length(.y) == 5 ~ "RB2"),
...
print_gini_ir_maps <- function(df, 
                               ano_inicial = 2006, 
                               ano_final = 2020, 
                               tabela = "RB2"){
  
  exemplo <- df %>% filter(tabela == tabela &
                             estado != "BR" &
                             !is.na(centil_1)) %>%
    group_by(ano, estado) %>%
    summarise(gini = ineq::Gini(media_do_centil))
  
  states <- read_state(year = 2019) #chama os sf dos estados brasileiros
  
  print(
    purrr::walk(ano_inicial:ano_final, ~ states %>% 
                 left_join(exemplo, by = c("abbrev_state" = "estado")) %>%
                 filter(ano == .x) %>%
                 ggplot() +
                 geom_sf(aes(fill=gini), color = NA, size = .15) +
                 labs(title = paste0("Gini da Renda de cada Estado Brasileiro no ano de ", .x),
                      subtitle = "Calculado a partir de dados de imposto de renda¹ declarados a Receita Federal", size = 8,
                      caption = "¹Renda tributável bruta e outras rendas isentas (RB2)") +
                 scale_fill_distiller(palette = "Blues", name = "Gini") +
                 theme_void())
  )
